...
#SBATCH --output=../logs/exp_123-gtaHR2csHR_fda_diss_src_ceorig_inv_trg_ceorigorig_invorigorigstylizedstylized-ablation_inv_weights-gpus2_distributed-slurm-05-01-%a.o
#
# Specify file for logging standard error.
#SBATCH --error=../logs/exp_123-gtaHR2csHR_fda_diss_src_ceorig_inv_trg_ceorigorig_invorigorigstylizedstylized-ablation_inv_weights-gpus2_distributed-slurm-05-01-%a.e
#
# Specify open mode for log files.
#SBATCH --open-mode=append
#
# Specify jobname and range of tasks for job array.
#SBATCH --job-name=exp_123-gtaHR2csHR_fda_diss_src_ceorig_inv_trg_ceorigorig_invorigorigstylizedstylized-ablation_inv_weights-gpus2_distributed-slurm-05
...
export CHECKPOINT_RESUME="/cluster/work/cvl/csakarid/results/DISS/local-exp120/230128_0939_gtaHR2csHR_1024x1024_dacs_a999_fdthings_diss_src_ceorig_inv_trg_ceorigorig_invorigorigstylizedstylized_rcs001-20_cpl2_hrda1-512-01_daformer_sepaspp_sl_mitb5_poly10warm_s0_afe24/latest.pth"
...
MASTERPORT_BASE="29539"
MASTERPORT_THIS_TASK=$((${MASTERPORT_BASE}+${SLURM_ARRAY_TASK_ID}))

# Run the experiment.
python -m torch.distributed.launch --nproc_per_node=2 --nnodes=1 --node_rank=0 --master_port=${MASTERPORT_THIS_TASK} run_experiments.py --exp ${EXP_ID} --resume-from ${CHECKPOINT_RESUME}
